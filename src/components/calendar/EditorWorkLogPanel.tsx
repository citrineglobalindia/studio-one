...
import { ClipboardList, Plus, Trash2, Check, Loader2, Pencil, X, Palette } from "lucide-react";
...
export function EditorWorkLogPanel({ dateIso, canManage }: { dateIso: string; canManage: boolean }) {
  const { logs, isLoading, add, update, remove } = useEditorWorkLogs(dateIso);
    ...
    <div className="rounded-2xl border border-border bg-card overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-border bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">{logs.length} entr{logs.length === 1 ? "y" : "ies"} · {dayTotal} items total</p>
          </div>
        </div>
        {canManage && <Button size="sm" onClick={() => setAdding(true)} className="h-7 gap-1 text-xs"><Plus className="h-3.5 w-3.5" /> Add</Button>}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-10 text-center">
          ...
          {canManage && <button onClick={() => setAdding(true)} className="text-xs text-primary hover:underline mt-1">Add the first entry</button>}
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-border">
              {grouped.map(([code, rows]) => {
                ...
                return rows.map((l, idx) => (
                  <LogRow key={l.id} log={l} canManage={canManage} showEditor={idx === 0}
              ...
              {adding && <AddRow dateIso={dateIso} onCancel={() => setAdding(false)} onSave={async (p) => { await add.mutateAsync(p as any); setAdding(false); }} />}
            </tbody>
          </table>
        </div>
      )}
...
function LogRow({ log, canManage, showEditor, editorRowSpan, editorTotal, showTotal, onUpdate, onRemove }: {
  log: DbWorkLog; canManage: boolean; showEditor: boolean; editorRowSpan: number; editorTotal: number; showTotal: boolean;
  onUpdate: (p: Partial<DbWorkLog>) => void; onRemove: () => void;
}) {
  ...
  return (
    <tr className="hover:bg-muted/20 align-middle">
      ...
      <td className="px-2 py-2 text-center">
        <button onClick={() => onUpdate({ is_done: !log.is_done })} className="inline-flex">
        ...
        <td className="px-2 py-2 text-right whitespace-nowrap">
          {editing ? (
            ...
            <div className="inline-flex gap-0.5">
              ...
              <Button size="icon" variant="ghost" className="h-6 w-6 text-rose-500" onClick={onRemove}><Trash2 className="h-3 w-3" /></Button>
            </div>
          )}
        </td>
      )}
...
function AddRow({ dateIso, onCancel, onSave }: { dateIso: string; onCancel: () => void; onSave: (p: any) => Promise<void> }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await onSave({ log_date: dateIso, editor_code: f.editor_code.trim(), editor_name: f.editor_name.trim() || null, client_name: f.client_name.trim() || null, work_type: f.work_type || null, work_count: Number(f.work_count) || 0, status: f.status, notes: f.notes.trim() || null });
