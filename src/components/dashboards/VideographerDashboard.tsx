...
export function VideographerDashboard() {
  ...
  const stats = [
    ...
    { label: "Pending Footage", value: pending, icon: Film, gradient: "linear-gradient(135deg,#fbbf24,#f59e0b)", glow: "shadow-amber-500/30" },
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="px-5 pt-5 space-y-4">
      <motion.div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(168,85,247,0.18) 0%, rgba(56,189,248,0.18) 50%, rgba(37,99,235,0.18) 100%)",
          boxShadow: "0 24px 60px -20px rgba(56,189,248,0.4), inset 0 1px 0 rgba(255,255,255,0.1)",
        }}
      >
        <div className="absolute -top-16 -right-12 w-48 h-48 bg-purple-400/30 rounded-full blur-3xl" />
        <div className="absolute -bottom-16 -left-10 w-40 h-40 bg-sky-500/30 rounded-full blur-3xl" />
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-1">
            <Video className="h-3.5 w-3.5 text-sky-300" />
            <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-sky-200/80">Videographer</span>
          </div>
          ...
          <p className="text-xs text-sky-100/80 mt-2">
            ...
            <span className="text-amber-300 font-semibold">{pending}</span> footage pending
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="divide-y divide-white/5">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-sm text-slate-400">No shoots assigned</div>
          ) : (
      ...
      <motion.div variants={cardVariants} className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: "View Calendar", icon: Film, path: "/m/calendar" },
        ].map((a) => (
          <button
            key={a.label}
          ...
          >
            <div
              className="h-10 w-10 rounded-xl flex items-center justify-center shadow-lg shadow-purple-500/30"
