  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="px-5 pt-5 space-y-4">
      <motion.div
        ...
        style={{
          background: "linear-gradient(135deg, #6366f1 0%, #2563eb 50%, #38bdf8 100%)",
      ...
      >
        ...
        <div className="absolute -bottom-16 -left-10 w-40 h-40 bg-purple-400/40 rounded-full blur-3xl" />
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-1">
            <Video className="h-3.5 w-3.5" />
            <span className="text-[10px] font-semibold uppercase tracking-[0.18em] opacity-90">Videographer</span>
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight">Welcome back 🎬</h1>
          <p className="text-xs opacity-90 mt-2">
            <span className="font-bold">{upcomingEvents.length}</span> upcoming shoots ·{" "}
            <span className="font-bold">{pending}</span> footage pending
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="divide-y divide-blue-50">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-sm text-slate-500">No shoots assigned</div>
          ) : (
      ...
      <motion.div variants={cardVariants} className="grid grid-cols-2 gap-3">
        {[
          ...
          >
            <div
              className="h-10 w-10 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/30"
